...
class TrilobeatGUI : public LV2::GUI<TrilobeatGUI, LV2::URIMap<true>, LV2::WriteOSC<true> > {
public:
  ...
  // XXX this never gets called now
...
protected:
  ...
  void send_names(std::string const& names) {
    ...
    write_osc(k_msg_input, "/new_names", "s", names.c_str());
  }

  void send_velocity(unsigned char key, unsigned step, unsigned char velocity) {
    write_osc(k_msg_input, "/velocity", "iii", key, step, velocity);
  }
