...
void Voice::set_region(const SampleBuffer& buffer, 
...
void Voice::render_to(float* left, float* right, uint32_t nframes, float gain) {
  ...
  if (m_chunk) {
    ...
    if (m_pos >= m_buffer->get_length()) {
      m_buffer = 0;
      return;
    }
    if (m_end > m_buffer->get_length())
      m_end = m_buffer->get_length();

    const float* data = m_buffer->get_data(0);
    for (uint32_t i = 0; i < nframes; ++i) {
      if (m_pos >= m_end)
	break;
      left[i] += gain * data[m_pos];
