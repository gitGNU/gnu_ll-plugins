/****************************************************************************
    ...
    sineshapergui.cpp - A LV2 synth plugin
...
void SineShaperGUI::set_bundle_path(const std::string& bundle) {
  m_bundle = bundle;
}


/* As I said, horrible. */
void SineShaperGUI::connect_all(LV2UIClient& dssi) {
...
void SineShaperGUI::program_selected(int program) {
...
SkinDial* SineShaperGUI::create_knob(const string& name, const string& tooltip,
  ...
  RefPtr<Pixbuf> pixbuf = 
    Pixbuf::create_from_file(m_bundle + "/dial.png");
...
void SineShaperGUI::save_preset() {
  cerr<<__PRETTY_FUNCTION__<<" is not implemented"<<endl;
  /*
...
void SineShaperGUI::factory_preset_selected() {
  ...
  if (iter)
    signal_select_program((*iter)[m_preset_columns.col_number]);
}
