  ...
  for (unsigned long i = 0; i < sample_count; ++i) {
    ...
    for ( ; iter != midi_end && (unsigned long)iter->timestamp() == i; ++iter) {
      ...
      if ((data[0] & 0xF0) == 0x90) {
        ...
        // update variables
        const unsigned char& key = data[1];
        m_pitch = m_table[key];
        m_velocity = data[2] / 128.0f;
        ...
        if (m_active_key == Key::NoKey || !tie_overlapping) {
      ...
      // pitchbend
        ...
        int value = -8192 + ((data[2] << 7) + data[1]);
        m_pitchbend = pow(twelfth_root_of_2, value / 4096.0f);
      }
      
      // all sound off
      else if ((data[0] & 0xF0) == 0xB0 && data[1] == 120) {
...
void initialise() {
  register_lv2<SineShaper>("http://ll-plugins.nongnu.org/lv2/dev/sineshaper/0.0.0");
}
